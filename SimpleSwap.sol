...
contract SimpleSwap {
    ...
    function swapExactETHForTokens(uint256 _amountOutMin, address _tokenOut, address _to, uint256 _allowedTime) external payable{
        ...
        _path[0] = router.WETH();
        _path[1] = _tokenOut;
        unchecked{router.swapExactETHForTokens{value: msg.value}(_amountOutMin, _path, _to, block.timestamp + _allowedTime);}
    }

    function swapExactTokensForTokens(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin, address _to, uint256 _allowedTime) external{
        ...
        _path[0] = _tokenIn;
        _path[1] = _tokenOut;
        unchecked{router.swapExactTokensForTokens(_amountIn, _amountOutMin, _path, _to, block.timestamp + _allowedTime);}
    }

    function withDrawStuckEth() external onlyOwner{
        (bool _callSuccess,) = payable(msg.sender).call{value: address(this).balance}("");
        if(!_callSuccess){
